...
| **SRAM Compiler** | OpenRAM (via Sky130 `sky130_sram_macros`) |
...
The data memory is built from **4 × `sky130_sram_1kbyte_1rw1r_8x1024_8`** macros — one per byte lane (4 KB total). These macros are generated by [**OpenRAM**](https://github.com/VLSIDA/OpenRAM), the open-source SRAM compiler. Pre-compiled copies ship with the Sky130 PDK under `sky130_sram_macros`, so no local OpenRAM run is needed to reproduce this flow.

| Property | Value |
|----------|-------|
| **Macro** | `sky130_sram_1kbyte_1rw1r_8x1024_8` |
| **Organization** | 1024 words × 8-bit (1 KB) |
| **Ports** | 1RW port + 1R port (1RW1R), write mask |
| **Instances** | 4 — one per byte lane of `data_memory` |

### How the macros enter the flow
1. **RTL** — `rtl/mem_stage/sram_1kbyte_1rw1r_8x1024_8_bb.v` provides an empty `(* blackbox *)` model whose pins exactly match the macro LEF (`addr0/addr1`, `din0`, `dout0/dout1`, `csb0/csb1`, `web0`, `wmask0`, `clk0/clk1`, `vccd1/vssd1`).
2. **Synthesis** — Yosys keeps the module as a blackbox; no internal netlist is synthesized.
3. **Place & Route** — `flow.tcl` reads the macro LEF and TT timing lib from the PDK (`libs.ref/sky130_sram_macros/{lef,lib}/…`), then OpenROAD binds the 4 instances to the LEF macro and places them with halos.

### Regenerating macros with OpenRAM (optional)
Only needed if you want a different size/org/port configuration:
```bash
git clone https://github.com/VLSIDA/OpenRAM && cd OpenRAM
# install per repo instructions, then run with a sky130 config file:
openram.py myconfig_sky130.py   # emits .v / .lib / .lef / .gds / .sp
```
...
- OpenRAM (optional — only to recompile custom SRAM macros)
...
- **OpenCores:** https://opencores.org/projects/rv32im-pipelined (SVN trunk, tag `v1.0-alpha`, LGPL-3.0)
